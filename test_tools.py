...
def setup_test_files():
    ...
    with open("workspace/input/test_read.txt", "w", encoding="utf-8") as f:
...
def test_file_search():
    ...
    res = file_tool.search_files("test_read", search_roots=["C:\\1 CODE\\CONTROLPC\\workspace\\input"])
...
def test_file_read_and_summarize():
    ...
    filepath = "C:\\1 CODE\\CONTROLPC\\workspace\\input\\test_read.txt"
    ...
    assert "Bản tóm tắt tệp tin:" in res_sum["summary"]
...
def test_docx_creation():
    ...
    output_path = "C:\\1 CODE\\CONTROLPC\\workspace\\output\\test_report.docx"
    ...
    paragraphs = [
        "Hoàn thành thiết lập nền móng cho dự án CONTROLPC.",
        "Xây dựng thành công Policy Engine và Permission Engine.",
        "Tích hợp các file tool và document tool kiểm soát rủi ro."
    ]
    ...
    assert "Báo cáo công việc tuần" in res_read["content"]
    assert "Xây dựng thành công Policy Engine" in res_read["content"]
